...
function URLrequest() {
	...
	commandData = {
		type: 1,
		url: 'https://www.vital-dahoim.de/',
		crawlqueue_id: '9994894689498489469',
		host_id: '4156456456456456'
	}
	doAction(commandData);

//  	http.get('http://XXX.XXX.XXX.XXX/vcx3crawler/backend/www/index.php?slave='+process.argv[2]+'&type='+process.argv[3], (resp) => {
//   	let data = '';
...
// 	commandData = JSON.parse(decodedata);
...
{  
	if (puppetPage)
	{
		...
		try {
		    ...
		    {
				myVar = setTimeout(function(){ myMethodCall() }, 40000);
			    await puppetPage.goto(commandData.url, {waitUntil: 'networkidle2'});
			    clearTimeout(myVar);
			    wait(500);
		    ...
		    if (commandData.type==2)
		    {
				...
				await Promise.all([
				      ...
				      puppetPage.waitForNavigation({ waitUntil: 'networkidle0' }),
				]);	        
				wait(1250);
			}
		    ...
		    {
				myVar = setTimeout(function(){ myMethodCall() }, 40000);
			    await puppetPage.goto(commandData.url, {waitUntil: 'networkidle0'});
			    clearTimeout(myVar);
			    wait(1250);
		    }
			...
			wait(500);
			data = await puppetPage.evaluate(() => document.querySelector('*').outerHTML);
			wait(250);
