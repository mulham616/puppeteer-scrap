...
function URLrequest() {
	try{
		http.get('http://info.cern.ch/?slave='+process.argv[2]+'&type='+process.argv[3], (resp) => {
		   let data = '';
...
{  
	if (puppetPage)
	{
		...
		try {
		    ...
		    {
			    await puppetPage.goto2(commandData.url, {waitUntil: 'networkidle2'}, 40000);
			    wait(500);
		    ...
		    if (commandData.type==2)
		    {
				...
				await Promise.all([
				      ...
				      puppetPage.waitForNavigation2({ waitUntil: 'networkidle0' }, 40000),
				]);	        
				wait(1250);
			}
		    ...
		    {
			    await puppetPage.goto2(commandData.url, {waitUntil: 'networkidle0'}, 40000);
			    wait(1250);
		    }
...
try{
	...
	try {
		...
		puppetPage.waitForNavigation2 = ( options, timeout ) => {
			try{
				return new Promise((resolve, reject) => {
					page.waitForNavigation( options ).then( resolve ).catch(e => reject(e))
					setTimeout( () => reject(`TimeoutError: Navigation timeout of ${timeout} ms exceeded`), timeout)
				})
			}catch(e){
				throw new Error('Network Error')
